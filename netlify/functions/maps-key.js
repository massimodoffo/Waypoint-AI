...
//
// No Referer/Origin check is added here on purpose — it wouldn't stop a
// determined caller (both headers are client-controlled) and would just add
// false confidence. The actual protection is Google's own referrer
// restriction on the key, enforced when the key is used against Google's
// APIs, not who's allowed to ask this function for it.
exports.handler = async function(event) {
  ...
  if (!key) {
    // 404, not 500 like proxy.js's missing ANTHROPIC_API_KEY — that one is
    // required for the app to function at all; this one is an optional
    // feature that's designed to degrade gracefully (see getGoogleMapsKey
    // in cards.js), so an unset key isn't a server misconfiguration.
